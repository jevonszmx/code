...
static PHP_MINIT_FUNCTION(json)
{
	...
	REGISTER_LONG_CONSTANT("JSON_IGNORE_NON_UTF8", PHP_JSON_IGNORE_NON_UTF8, CONST_CS | CONST_PERSISTENT);
...
static PHP_MINFO_FUNCTION(json)
{
	...
	php_info_print_table_row(2, "json remark", "support for non utf8 charset, like 5.3, modify by jevonszmx@gmail.com");
...
static void json_escape_string(smart_str *buf, char *s, int len, int options TSRMLS_DC) /* {{{ */
{
	...
	if (ulen <= 0) {
		...
		if (ulen < 0) {
			if ((options & PHP_JSON_IGNORE_NON_UTF8)) {
				php_error_docref(NULL TSRMLS_CC, E_WARNING, "There ara non utf8 string!");
			}
			else {
...
static PHP_FUNCTION(json_last_error_msg)
{
	...
	switch(JSON_G(error_code)) {
		...
		case PHP_JSON_ERROR_UTF8:
			RETURN_STRING("Malformed UTF-8 characters, possibly incorrectly encoded, you can use JSON_IGNORE_NON_UTF8", 1);
